<script setup>
    ...
    fetch("/api/users/orders", {
        ...
        orders.value = {ok: true, data: all_orders};
    }).catch((err) => {
    ...
    function deleteOrder() {
        const username = orderToDelete.value.username;
        const order_id = orderToDelete.value.id;
        fetch(`/api/users/u/${username}/orders/${order_id}`, {
            ...
            if (res.ok) {
                for (let i = 0; i < orders.value.data.length; i++) {
                    if (orders.value.data[i].id === orderToDelete.value.id) {
            ...
            <div v-else-if="orders.ok" class="rounded-md border border-[var(--color-prussianblue)] overflow-hidden">
                <table class="box-border text-[var(--color-prussianblue)] table-auto w-full">
                    ...
                    <tbody>
                        <tr v-for="order in orders.data" class="not-last:border-b-1 h-fit hover:bg-[var(--color-prussianblue)]/15">
                            ...
                            <td class="px-2 py-1 hover:cursor-pointer border-l-1 border-[var(--color-prussianblue)]" @click="showOrderInformation(order)">{{ order.id }}</td>
                            <td class="px-2 py-1 hover:cursor-pointer" @click="showOrderInformation(order)">{{ order.username }}</td>
                    ...
                    <div class="flex flex-col items-center gap-4 box-border text-[var(--color-prussianblue)]">
                        <span class="flex flex-col items-center">
                            Do you want to delete the order with id 
                                <span> <b>{{ orderToDelete.id }} </b> ? </span>
                        </span>
                ...
                <dialog class="m-auto py-[1vh] px-[2vw] rounded-md" ref="orderDialog">
                    <div class="flex flex-col gap-[1vh]">
                            ...
                            <table class="text-left">
                                <tbody>
                                    <tr class="border-b border-[var(--color-prussianblue)]">
                                        ...
                                        <td class="py-1 px-2">{{ orderDetailed.id }}</td>
                                    </tr>
                                    <tr class="border-b border-[var(--color-prussianblue)]">
                                    ...
                                    <tr class="border-b border-[var(--color-prussianblue)]">
                                        <th class="bg-[var(--color-prussianblue)] text-white py-1 px-2">User ID</th>
                                        <td class="py-1 px-2">{{ orderDetailed.user_id }}</td>
                                    </tr>
                                    <tr class="border-b border-[var(--color-prussianblue)]">
                                        <th class="bg-[var(--color-prussianblue)] text-white py-1 px-2">Username</th>
                                    <td class="py-1 px-2">{{ orderDetailed.username }}</td>
                                    </tr>
                                    <tr class="border-b border-[var(--color-prussianblue)]">
                                        <th class="bg-[var(--color-prussianblue)] text-white py-1 px-2">Name</th>
                                        <td class="py-1 px-2">{{ orderDetailed.name }}</td>
                                    </tr>
                                    <tr class="border-b border-[var(--color-prussianblue)]">
                                        <th class="bg-[var(--color-prussianblue)] text-white py-1 px-2">Surname</th>
                                        <td class="py-1 px-2">{{ orderDetailed.surname }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <a @click="orderDialog.close(); error_message='';" class="btn-pb-b w-fit mx-auto">Close</a>
                    </div>
                </dialog>
            </div>
            <div v-else>
