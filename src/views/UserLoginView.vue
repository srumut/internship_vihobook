<script setup>
    ...
    const submitForm = () => {
        ...
        .then(async (res) => {
            ...
            document.cookie = `account=user; expires=${exp.toUTCString()}; path=/`;
    ...
    <div class="h-dvh flex items-center justify-center">
        <div class="m-4 w-full sm:w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
            ...
            <form @submit.prevent="submitForm"  class="grid grid-cols-[27%_70%] gap-4">
                ...
                <input id="password" v-model="password" type="password" autocomplete="new-password" class="input-pb" required/>

                <button type="submit" class="col-span-2 btn-bg-b">Login</button>
            </form>
        </div>
    </div>
</template>